...
[#12](https://github.com/ajmichels/moneta-notes/issues/12) asks for a way to flag certain
files/directories in the vault as read-only by glob pattern, so an agent (or the CLI) can't
accidentally mutate content that's meant to stay fixed — vendored reference material, a symlinked
external source ([#11](https://github.com/ajmichels/moneta-notes/issues/11)), a template folder, an
archive of past work. This spec defines: the `.mnotesreadonly` pattern file, the guard every mutating
tool now runs before touching disk, and how read-only status is surfaced back to a caller on every
read/list tool so it's visible *before* a write is attempted, not just discoverable by hitting the
guard.

This is deliberately modeled on `.mnotesignore` (S010) — same file location (vault root), same
syntax (gitignore, via the `ignore` npm package), same "absent file = no-op" posture — but it is a
**separate file and a separate mechanism**, not an extension of `.mnotesignore`'s. `.mnotesignore`
answers "is this in the index at all"; `.mnotesreadonly` answers "can the tool surface write to this,"
which is an orthogonal question — a read-only note is still fully indexed, searchable, and readable,
exactly like any other note. Conflating the two into one file would force every `.mnotesignore`
pattern to also mean "and don't let anyone write here," which isn't what excluding a folder from the
index means today and shouldn't silently start meaning tomorrow.

## `.mnotesreadonly`

A gitignore-style file at the vault root, same syntax `.mnotesignore` already uses (negation, `**`,
directory-only trailing-slash patterns, comments — the full `ignore` package grammar, not a hand-rolled
subset). An absent file behaves as empty — nothing is read-only by default, matching `.mnotesignore`'s
own absent-is-empty posture.
...
- **`loadReadonlyMatcher(vaultRoot) -> matcher`** — identical shape to `loadIgnoreMatcher`: reads
  `.mnotesreadonly` at the vault root via the `ignore` package, returns a matcher. Absent file → empty
  matcher.
- **`checkReadonly(matcher, relativePath) -> { readonly: boolean, pattern: string|null }`** — wraps
  ...
  version). `pattern` is the matched rule's literal text when `readonly` is `true` (`ignore`'s
  `.test()` returns `{ ignored, rule: { pattern } }` — this just projects that down to what callers
  here need), `null` otherwise. Takes an already-loaded `matcher`, not `vaultRoot`, specifically so a
  caller iterating many rows (`search`, `grep`, `tag_notes`, `metadata_query`) loads the file once per
  call and tests each row against the same instance, rather than re-reading `.mnotesreadonly` per row.
- **`assertWritable(vaultRoot, relativePath)`** — the write-time guard. Loads the matcher itself
  ...
  Unwrapped by its caller, same precedent `resolveVaultPath`/`titleToPath` already set (a vault-
  containment violation surfaces as `resolveVaultPath: ...`, not re-prefixed with the calling tool's
  name) — every mutator below calls this with no try/catch, letting the message propagate verbatim.
...
nothing about read-only status needs to be table-backed. `assertWritable` and `checkReadonly` both
load `.mnotesreadonly` fresh on every call — a small file, cheap to parse at this project's stated
scale (thousands of notes, not millions). This means editing `.mnotesreadonly` takes effect
**immediately**, everywhere, with no daemon restart or reindex required — a deliberately simpler
freshness story than `.mnotesignore`'s, made possible by the fact that read-only status is a flag on
an already-selected row/path, not a gate on what enters the index in the first place.
...
comparison, or disk I/O.** This is the earliest point every one of these functions has a concrete
vault-relative path to check, and checking first means a caller targeting a read-only note gets that
specific answer rather than, say, a hash-mismatch error that would've resolved differently against a
writable note.
...
No new CLI/MCP logic is needed to enforce this — per CLAUDE.md's architecture rule, the guard lives
entirely in `core/`, so both surfaces inherit it automatically through their existing thin wrappers
around these five functions. A blocked call surfaces through the same paths every other thrown `core/`
error already does: MCP's `isError: true` response with the message preserved verbatim (S007), the
CLI's non-zero exit with the message on stderr, and an `outcome: 'error'` line in `audit.log` (S008) —
nothing new to build for any of that; `assertWritable`'s thrown `Error` is indistinguishable, plumbing-
wise, from a hash-mismatch or size-drop-guard error that already flows through these same paths.
...
exception, decided as follows: a read-only note left with a broken `[[wikilink]]` after a rename
elsewhere is a worse outcome than the cascade silently fixing it — Obsidian shows a dangling link with
no indication of where the target moved, and may prompt to create a new (wrong) note at the stale
name if the user clicks through. Protecting a read-only note's *content* from a direct caller-initiated
write and keeping its *outbound links* correct as an automatic consequence of a rename elsewhere are
different concerns; only the first is what `.mnotesreadonly` is for.
...
initiated mutation). The same reasoning extends to the read-only guard for the same reason.
...
Every read/list tool surfaces read-only status **before** a caller would otherwise have to hit the
write guard to find out. Two shapes, matching the two output-format families already established
(S007's "Output formats"):
...
column, following the exact convention you'd expect from `formatCell` (`src/format.js`, unchanged):
**the literal string `read-only` when true, nothing (empty cell) otherwise.** This is a two-layer
split, keeping `core/` and `format.js` on their existing sides of the formatting boundary
(CLAUDE.md):
...
CLI layer, not duplicating any logic that lives elsewhere (there is no other copy of this check to
duplicate). `core/links.js` itself is unchanged.

## MCP tool descriptions (amending S007)

Per the existing precedent for behavior a caller needs to know about to use the tool surface correctly
...
- **Per-note frontmatter `readonly: true` flag** as an alternative or complement to glob patterns.
  ...
  based control, matching how `.mnotesignore` already works; a frontmatter-flag mechanism is a
  different design (caller-visible/settable per-note state vs. an external policy file) worth its own
  discussion if it comes up, not folded in here.
