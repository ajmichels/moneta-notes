...
| [S015 — Read-only Paths](docs/specs/S015-readonly-paths.md) | `.mnotesreadonly` glob patterns, the write-time guard across every mutating tool, and the `readonly` field/column every read/list tool reports |
...
server, logging, config & install, links, attachments, vector tools, metadata search, read-only paths.
